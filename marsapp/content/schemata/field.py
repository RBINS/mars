...
class CoordinatesField(RecordField):
    """
    """
    ...
    def getSubfieldViews(self,instance,joinWith=', '):
        ...
        for subfield in self.getSubfields():
            ...
            if view:
                result.append(view)
                result[-1] = self.getSubfieldLabel(subfield) + ': ' + result[-1]
        return result

from Products.Archetypes.Registry import registerField

registerField(CoordinatesField,
              title="Coords",
              description="Used for storing a 'record' (dictionary-like) construct, small difference in output compared to recordfield.",
              )
